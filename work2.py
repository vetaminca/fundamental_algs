...
# 1️⃣ Определим буквы, участвующие в уравнении
...
# 2️⃣ Мы знаем, что D = 5 по условию
fixed = {'D': 5}

# 3️⃣ Ведущие буквы (первые в словах): D, G, R — не могут быть нулём
leading_letters = {'D', 'G', 'R'}

# 4️⃣ Определим оставшиеся буквы (те, для которых нужно подобрать цифры)
remaining_letters = [l for l in letters if l not in fixed]
print("Неизвестные буквы:", remaining_letters)

# 5️⃣ Все возможные цифры 0–9, кроме уже занятых
digits = set(range(10)) - set(fixed.values())

# 6️⃣ Вспомогательная функция для подстановки букв в число
...
# 7️⃣ Перебираем все возможные комбинации цифр для оставшихся букв
